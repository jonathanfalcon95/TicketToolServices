...
namespace TicketToolServices.Repository
{
    ...
    {
        public static async Task Post(Tickets tickets, ExecutionContext context, ILogger log, bool debuging)
        {
            try
            {
                ...
                using (SqlConnection sql = new SqlConnection(str))
                {
                    using (SqlCommand cmd = new SqlCommand("dbo.sp_Insert_Ticket", sql))
                    {
                        ...
                        if (debuging)
                        {
                            log.LogInformation("SP_Ticket executed on Ticket " + tickets.TicketID );
                        }
                        return;
                    }
                }
