...
namespace TicketToolServices.Controllers
{
    public static class FunctionTickets
    {
        ...
        {
        /*    var response = Conexion.GetDataApi("/tickets", "?include=description,stats");
          ...
          // var response2 = Conexion.GetDataApi("/tickets");

            var DateExecute = await SFControlDataRepository.SelectAsync(context);


            log.LogInformation("paso algo=================================================");
                var dataObjects = DateExecute.ToString();
                {
                    foreach (var item2 in dataObjects)
                    {
